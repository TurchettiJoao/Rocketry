...
N = 50*3; % O segundo número representa a duração da simulação
...
m = 3; % Massa do foguete em kg
L = 0.5; % Distância entre o Cg e a hélice em metros
LT = 1; % Altura do foguete
F = 100; % Empuxo da hélice em Newtons
R = 98/2/1000; % Raio do foguete em metros
In = m*R*R/4 + m*LT*LT/12; % Momento de inércia do foguete
...
pitch_inicial = 25*pi/180; % Ângulo do foguete em relação à vertical, pode variar de -pi/2 a pi/2
...
%   wx(i) = wx(i-1) + dt*ang_acc_x(i-1);
    ...
    erro(i) = pitch_referencia - pitch(i) + randn()/10; % COM RUIDO
